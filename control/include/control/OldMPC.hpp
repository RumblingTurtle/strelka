...
//   [ c,  0, -a]
//   [-b,  a,  0]
void ConvertToSkewSymmetric(const Eigen::Vector3d &vec,
...
public:
  ...
  Mpc(double mass, const std::vector<double> &inertia, int num_legs,
      int planning_horizon, double timestep,
      const std::vector<double> &qp_weights, double alpha = 1e-5);

  virtual ~Mpc() { osqp_cleanup(workspace_); }
  ...
  // -> X".
  std::vector<double> &
  ComputeContactForces(MatrixXd &_mpc_table, MatrixXd &_foot_table,
  ...
  void CalculateABExponentials();

  void CalculateQpMatsProper();
...
private:
  ...
  Eigen::MatrixXd a_qp_new_;  // 13 * horizon x 13
  Eigen::MatrixXd b_qp_new_;  // 13 * horizon x num_legs * 3 * horizon sparse
  Eigen::MatrixXd p_mat_;     // num_legs * 3 * horizon x num_legs * 3 * horizon
  Eigen::MatrixXd p_mat_new_; // num_legs * 3 * horizon x num_legs * 3 * horizon
  ...
  Eigen::MatrixXd b_exps_; // 13 * horizon x (num_legs * 3 )
