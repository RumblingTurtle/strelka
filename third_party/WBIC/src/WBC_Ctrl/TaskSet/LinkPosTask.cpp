...
LinkPosTask<T>::LinkPosTask(const FloatingBaseModel<T> *robot, int link_idx,
                            bool virtual_depend)
    : Task<T>(3), robot_sys_(robot), link_idx_(link_idx),
      virtual_depend_(virtual_depend) {
  TK::Jt_ = DMat<T>::Zero(TK::dim_task_, robot->dim_config);
...
template <typename T> bool LinkPosTask<T>::_UpdateTaskJacobian() {
...
template <typename T> bool LinkPosTask<T>::_UpdateTaskJDotQdot() {
