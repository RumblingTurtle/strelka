...
#include <WBIC/WBC_Ctrl/ContactSet/FixedBodyContact.hpp>
...
template <typename T> bool FixedBodyContact<T>::_UpdateJc() { return true; }

template <typename T> bool FixedBodyContact<T>::_UpdateJcDotQdot() {
  Contact::JcDotQdot_ = DVec<T>::Zero(Contact::dim_contact_);
  return true;
}

template <typename T> bool FixedBodyContact<T>::_UpdateUf() { return true; }
