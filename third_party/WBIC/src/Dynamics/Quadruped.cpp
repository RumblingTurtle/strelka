...
bool Quadruped<T>::buildModel(FloatingBaseModel<T> &model) {
  ...
  for (int legID = 0; legID < 4; legID++) {
    ...
    Mat6<T> xtreeHipRotor =
        createSXform(I3, withLegSigns<T>(_hipRotorLocation, legID));
    if (sideSign < 0) {
    ...
    if (sideSign < 0) {
      ...
      model.addGroundContactPoint(
          bodyID, Vec3<T>(0, _kneeLinkY_offset, -_kneeLinkLength), true);
    } else {
    ...
    // add foot
    // model.addGroundContactPoint(bodyID, Vec3<T>(0, 0, -_kneeLinkLength),
    // true);
...
template <typename T> FloatingBaseModel<T> Quadruped<T>::buildModel() {
  ...
  /*
    ...
    for (int legID = 0; legID < 4; legID++) {
      ...
      //              const Mat6<T>& Xtree, const Mat6<T>& Xrot);
      bodyID++;
      Mat6<T> xtreeAbad = createSXform(I3, withLegSigns<T>(_abadLocation,
    legID)); Mat6<T> xtreeAbadRotor = createSXform(I3,
    withLegSigns<T>(_abadRotorLocation, legID));
      ...
      if (sideSign < 0) {
        ...
        model.addBody(_kneeInertia, _kneeRotorInertia, _kneeGearRatio, bodyID -
    1, JointType::Revolute, CoordinateAxis::Y, xtreeKnee, xtreeKneeRotor);
      }
